...
static private void computeClasses(Spec spec)
...
#if DEBUG
	    Console.WriteLine("Adding char "+Int32.ToString(nextcls-1)+" split="+Int32.ToString(split)+" i="+Int32.ToString(i));
#endif
	    }
	  ccls[i] = (int) h[split];
	  }
  ...
  Console.WriteLine();
  Console.WriteLine("NFA has "+int.ToString(nextcls)+" distinct character classes.");
