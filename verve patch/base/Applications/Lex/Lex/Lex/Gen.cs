...
private void userDeclare()
  ...
  while (!ibuf.GetLine())
    ...
    Console.WriteLine("Line number " +
		      Int32.ToString(ibuf.line_number) + ":"); 
...
#endif
      nfa = (Nfa) o;
      Console.Write(Int32.ToString(spec.nfa_states.IndexOf(nfa)) + " ");
      }
...
private bool expandMacro()
  ...
  while ('}' != ibuf.line[elem])
    ...
    if (elem >= ibuf.line_read)
      {
      Error.impos("Unfinished macro name at line "
		   + Int32.ToString(ibuf.line_number));
      return ERROR;
      }
    }
...
private void saveStates()
  ...
  while (ibuf.line_index < ibuf.line_read)
    {
#if OLD_DEBUG
    Console.WriteLine("line read " + Int32.ToString(ibuf.line_read)
		      + "\tline index = " + Int32.ToString(ibuf.line_index));
#endif
    ...
    Console.WriteLine("Integer index \"" 
		      + Int32.ToString(spec.states.Count)
		      + "\".");
#endif
...
public int Advance()
  ...
  if (ibuf.line_index > ibuf.line_read)
    {
    Console.WriteLine("ibuf.line_index = " + Int32.ToString(ibuf.line_index));
    Console.WriteLine("ibuf.line_read = " + Int32.ToString(ibuf.line_read));
    Utility.assert(ibuf.line_index <= ibuf.line_read);
    }
...
private void print_header()
  ...
  foreach (string state in spec.states.Keys)
    ...
    if (DTrans.F != spec.state_dtrans[index])
      ...
      Console.WriteLine("\tStart index in transition table: "
			+ Int32.ToString(spec.state_dtrans[index]));
      }
    else
      {
  ...
  for (int i = 0; i < spec.dtrans_list.Count; ++i)
	...
	{
	Console.Write(" * State " + Int32.ToString(i) + " [nonaccepting]");
	}
      else
	{
	Console.Write(" * State " + Int32.ToString(i)
		      + " [accepting, line "
		      + Int32.ToString(dtrans.GetAccept().line_number)
		      + " <"
	...
	Console.Write(" * State " + Int32.ToString(i)
		      + " [accepting, line "
		      + Int32.ToString(accept.line_number)
		      + " <"
		      + accept.action
		      + ">]");
	...
	if (last_transition != dtrans.GetDTrans(j))
	  {
	  Console.Write("\n *    goto " + Int32.ToString(dtrans.GetDTrans(j)) + " on ");
	  chars_printed = 0;
	  }
