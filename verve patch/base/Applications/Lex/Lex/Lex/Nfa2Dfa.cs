...
public static void MakeDFA(Spec s)
  ...
  free_nfa_states(s);
#if OLD_DUMP_DEBUG
  Console.WriteLine(int.ToString(s.dfa_states.Count)
...
Console.WriteLine("done init");
  ...
  for (int istate = 0; istate < nstates; istate++)
	...
	dt.SetDTrans(i, nextstate);
      ...
      sb1.Append("Current count = "+int.ToString(s.dtrans_list.Count)+"\n");
      for (int i1 = 0; i1 < dt.GetDTransLength(); i1++)
	sb1.Append(int.ToString(dt.GetDTrans(i1))+",");
      sb1.Append("end\n");
...
public static void Print_Set(ArrayList nfa_set)
  ...
  for (elem = 0; elem < size; ++elem)
    ...
    Console.Write(Int32.ToString(nfa.GetLabel()) + " ");
    }
  }
#endif
}
